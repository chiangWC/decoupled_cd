...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and row-align a seed-42 external benchmark.")
...
def external_args(args: argparse.Namespace) -> SimpleNamespace:
    common: dict[str, Any] = {
        ...
        "seed": 42,
        "experiment_name": f"external_{args.model}_{args.dataset}_{args.dataset_variant}",
    }
    if args.model == "orcdf":
        common.update(
            {
...
def main() -> None:
    args = parse_args()
    if args.seed != 42:
        raise ValueError("External benchmarks are fixed to seed=42.")
