...
from data.pool_protocol import exact_row_overlap, group_overlap, sha256_file
from scripts.evaluate_coverage_slice import add_target_coverage


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit prepared pool datasets before external admission.")
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--registry", default="configs/external_benchmark_registry.json")
...
def main() -> None:
    ...
    if registry["model_seed"] != 42 or registry["split_seed"] != 2024:
        raise RuntimeError("External benchmark registry seed invariants changed.")
    reports = [
        audit_dataset(
            dataset,
