...
from ORCDF.plugin import DecoupledORCDF, build_concept_graph, build_tkc_mask
...
def main():
    ...
    model = DecoupledORCDF(
        ...
        decouple=args.plugin_decouple, aux_weight=args.plugin_aux_weight,
    ).to(device)
    ...
    def train_epoch(epoch):
        ...
        for stu, exer, kn_emb, label in trainer.train_loader:
            ...
            pred, extra = model(stu, exer, kn_emb, label=label)
    ...
    with open(os.path.join(args.log_dir, "metrics.json"), "w") as f:
        json.dump({**metrics, "decouple": args.plugin_decouple, "aux_weight": args.plugin_aux_weight}, f)
