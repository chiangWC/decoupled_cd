...
- 因而 $\beta$ 由“同一学生面对不同覆盖题目”与“同一题目被不同覆盖学生作答”的交叉变化识别。
- $\beta>0$ 表示目标局部覆盖越缺失，外部模型预测损失越高。
...
- 固定效应点估计使用交替去均值求解。
- 95% CI 使用固定全样本 FE 投影后的 student-cluster multiplicity bootstrap 近似：对学生充分统计量做 multinomial 重采样，固定 2,000 次、`seed=2024`；每个 replicate 不重新拟合固定效应，因此不称为 refitted pairs bootstrap。
