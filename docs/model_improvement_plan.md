...
7. 若继续沿实验 51 的 readout expert 底座推进，默认保留原版 full-trigger 作为基座；只有在出现更明确的 targeted slice 假设或更局部的引导目标时，才再访这条线。
8. 对已经系统复访但未形成 clean overall gain 的 readout / propagation / ranking-loss 路线，默认不再高优先级继续；只有在出现明确的新假设、且机制上明显区别于已失败版本时，才考虑重开。
9. 若用户明确要继续训练协议优化，当前唯一仍值得优先复访的候选是实验 37 那条 `exp/training-modes` 支线；否则默认优先继续模型结构改动。
10. 新结构先跑单次；单次值得继续时再补 `2-3` 个 seed。
11. 判断是否值得继续时，默认主看 `AUC/ACC`，并优先寻找至少 `1e-3` 量级的改善。
12. `RMSE/Brier/ECE` 与分桶校准默认只用于判断副作用；若主指标不成立，通常不要因次要指标小幅改善而继续扩线。
