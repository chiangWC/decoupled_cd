...
这部分回答的是: 哪些路线在实验 12 的旧主线口径下还有信号，但目前还不该直接视为对实验 21 主线底座的结论。

### C1. 在实验 12 旧主线上接近、但尚未在实验 21 上系统复验

这几条路线的原始对比对象都是实验 12，而不是实验 21。它们可以作为后续主线口径下的复验候选，但不能直接称为“接近当前主线”。
...
### C5. 实验 21 主线底座上的新结果与其 follow-up
...
- 实验 22: 将实验 17 的 `local UKC neighbor` 叠到实验 21 后，三 seed 均值约 `0.7484`，低于实验 21 主线底座 `0.7502`，当前不建议继续推进。
- 实验 23: 修正 `TKC` 行为项全局二次缩小后，三 seed 均值约 `0.7597`，显著高于实验 21 主线底座 `0.7502`。
