...
- active pure-CDM single-checkpoint trial runner / 当前默认实验口径: 实验 110，`scripts/run_assist09_history_alignment_trial.sh`，双塔 `64x80` + `dual_cdm_branch_bce_weight=0.18` + `recompute_minibatch batch=65536 lr=3e-4`。四 seed AUC `0.778252/0.778263/0.777449/0.779321`，mean `0.778321`，stdev `0.000665`，peak CUDA `6.19GB`。
- coverage-bias diagnostic: 实验 116 使用 `split_student_concept_holdout.py` 默认参数生成 ASSIST09 student-concept holdout stress split。四 seed Exp81 vs Exp110 full 支持低覆盖叙事：Exp81 mean low AUC `0.723470` vs full AUC `0.768472`，coverage gap `0.045001`；Exp110 full mean overall AUC `0.755054`，相对 Exp81 `+0.010490`，low AUC `0.733920`，相对 Exp81 `+0.010450`，并把 gap 收窄到 `0.042869`。四 seed ablation extension 显示 w/o cognitive alignment low AUC `0.731073`、w/o dual tower `0.730243`，均低于 full；w/o branch BCE 四 seed mean low AUC `0.671777`，主要由 seed2027 近随机 collapse 拉低，seeds 2024-2026 不复现随机崩，因此 branch BCE 应讲 stability component。该 coverage-bias 结论应绑定到 student-concept holdout / UKC-heavy stress split，不绑定默认随机 split。实验 119 的 seed2027 `single96` capacity control 接近 overall AUC 但 low AUC 和 hide80 robustness 明显弱于 Exp110 full，因此当前 dual-tower stress/robustness story 不应解释为简单 capacity scaling。实验 124 补 ASSIST17/NIPS34 单 seed 粗粒度模块消融后，cognitive alignment 和 dual-branch 仍可作为 story-facing/stability 组件，但 evidence-aware readout 在两个数据集上去掉反而提升，不能再作为核心跨数据集正贡献模块。
- history-hiding robustness diagnostic: 实验 117 对 holdout split 和默认 ordered split 做 evaluation-only student-history interaction hiding。seed2027 两个数据集都显示 Exp110 full 比 Exp81 有更高 hidden AUC 和更小 delta AUC；hide80 时 holdout Exp110 hidden AUC `0.717703` vs Exp81 `0.709184`，delta `0.036198` vs `0.037952`，ordered Exp110 hidden AUC `0.734223` vs Exp81 `0.725930`，delta `0.045098` vs `0.046751`。w/o dual tower 在两个 split 都明显更脆弱，hide80 delta 为 `0.051125/0.056712`，可作为双塔鲁棒性诊断。ASSIST09 holdout Exp81-vs-Exp110 full 现已补四 seed：hide80 Exp110 hidden AUC `0.716900` vs Exp81 `0.707375`，hidden Brier/ECE `0.199971/0.068623` vs `0.209758/0.100899`；但 delta AUC `0.038154` 略大于 Exp81 `0.037188`，所以多 seed holdout robustness 叙事强调 hidden-history 绝对性能和校准，不宣称 delta 更小。
- historical heavy reference: 实验 104 是前一版 active runner，双塔 `64x80` + `dual_cdm_branch_bce_weight=0.10`，四 seed mean `0.778370`；实验 106 在实验 104 底座上将 branch BCE 提升到 `0.18`，四 seed mean `0.778579`，但它是 heavy full-batch 口径。实验 110 继承实验 106 的结构设置并用 recompute minibatch 降低显存，现已提升为默认。
...
- 104 之后的实验主因: 双塔 `64x80` 虽然过 `0.778`，但 full-batch peak CUDA 约 `11.76GB`，提升相对成本有限。当前默认改为实验 110，即在 `~7GB` 显存水平下保留接近 104/106 的 pure-CDM 信号。
- 当前 low-memory pure-CDM default: 实验 110，`dual64x80 + branchBCE=0.18` 改为 `recompute_minibatch`，`batch=65536`、`lr=3e-4`，四 seed mean AUC `0.778321`、peak CUDA `6.19GB`。实验 111 已按用户要求排除 checkpoint inference average，对 exp110 做完整 Track A-D 消融；最高 mean-AUC 变体是 `batch=131072, lr=3e-4`，mean AUC `0.778536`（相对 exp110 `+0.000215`）但 peak CUDA `8.76GB`，所以只作为更高显存候选。exp110 仍是默认低显存口径；最高单 seed 是 `c_prior_start001 seed2027 = 0.779377`，但四 seed mean 基本中性，不能作为默认。实验 111 同时确认 branch BCE、cognitive alignment、dual tower 都是核心组件，移除后三者分别掉 `0.001840/0.003924/0.003665` AUC。
...
- 当前 practical sprint target 已按用户口径调整为 `test_auc >= 0.778`；`0.780` 仍是 desirable headroom。当前 active runner 实验 110 已作为 `~7GB` 默认口径过线；实验 102 fixed checkpoint average mean `0.778872` 仍是 pure-CDM evaluator 上界；实验 99 hybrid stacker 三 seed mean `0.786910` 仍是最高绝对值但不是默认 CDM 路线。
- 后续若继续 pure-CDM 论文路线，默认从当前 active runner 实验 110 出发。论文式 seed2027 消融默认使用 exp110 `baseline_reproduce`，AUC `0.7793212012`；不要把 `c_prior_start001` 的 `+0.000055` single-seed near-tie 当作默认。Heavy-route ablation 可补 `64x64/64x80/80x80`、branch 单独 AUC、融合 AUC、branch BCE 邻域解释、以及 `cognitive/guess/slip` 组件语义稳定性。
...
- 当前已吸收的最新结构更新:
  ...
  - 实验 104: 在 `exp/pure-cdm-default-promotion` 上新增单 checkpoint 双塔 `DecoupledCDMEnsemble` 并合入 `exp/trellis-trial`。主塔 `concept_dim=64`、副塔 `concept_dim=80`，模型内概率平均；`dual_cdm_branch_bce_weight=0.10` 直接监督两个 tower 的 branch 输出。四 seed AUC `0.778773/0.778250/0.778508/0.777948`，mean `0.778370`、stdev `0.000306`，相对实验 95 mean `+0.002091`，也高于 pre-dual single-checkpoint base mean `0.776736`。它是前一版 active runner 和 exp110 的历史对照，不再是当前默认
  ...
  - 实验 106: 继续从 exp104 双塔出发，只扫 pure-CDM branch BCE 权重。`0.18` 四 seed AUC `0.778890/0.778552/0.778256/0.778618`，mean `0.778579`、stdev `0.000226`，高于 exp104 `0.778370` 且更稳；`0.15` 给出最高峰值 `0.779046`，`0.20` mean `0.778561` 但 seed2026 尾部更弱。它是 exp110 的 heavy full-batch 起点，不是当前默认 runner 本身
...
- 当前正向支线候选:
  - 实验 110
    - branch/source: `exp/pure-cdm-distillation`，已同步到 `exp/trellis-trial`
    - 判断: 当前 active runner / 默认实验口径；低显存 single-run/single-checkpoint pure-CDM 路线
    - runner: `scripts/run_assist09_history_alignment_trial.sh`
    - config: `dual64x80 + branchBCE=0.18 + recompute_minibatch batch=65536 lr=3e-4`
    - 关键指标: seed2024 `0.778252`、seed2025 `0.778263`、seed2026 `0.777449`、seed2027 `0.779321`，mean `0.778321`，peak CUDA `6.19GB`
    - 限制: mean AUC 略低于实验 106 heavy full-batch `0.778579`，但显存大幅降低；exp111 的 `c_prior_start001` 单 seed peak 不替代这个 baseline
    - 详细指标见 `docs/experiments/110_recompute_minibatch_low_memory_dual_cdm.md`
  - 实验 106
    - branch/source: `exp/lightweight-single-tower-cdm`
    - 判断: 已记录的 heavy full-batch pure-CDM refinement；不依赖 fixed checkpoint average、valid-trained combiner 或 hybrid tabular side-channel；作为实验 110 的结构起点
    - runner: 历史复现可用 detail doc 命令；当前 active runner 已改为实验 110
  ...
  - 实验 104
    ...
    - 限制: 解释性低于单塔 CDM，因为最终输出是两个 CDM tower 的结构化平均；但每个 tower 仍保留 `cognitive/guess/slip` 分解，且 branch BCE 让 branch-level 输出本身可监督。实验 119 已补 seed2027 `single96` capacity control：overall AUC 接近但 low-coverage AUC 与 history hiding 明显弱于 Exp110 full；若论文只需要 compact diagnostic，暂不必扩 `single128` 或多 seed，若要强 capacity/fairness 表再扩展。
...
- 最近失败或已暂停的 follow-up:
  ...
  - 实验 105: shared-branch single-tower 与 single72 capacity probe 均低于 exp104 pre-dual single64 reference，也不替代当前实验 110 默认口径；若显存是硬约束，可用 single64 reference，但不要把 shared-branch 或 dim72 设为长期默认路线
...
1. 当前 Trellis-managed worktree 以后从 `exp/trellis-trial` 伪主线或其后代出发；`master` 只作为模型主线语义和 accepted state 参考。当前 practical sprint target 是 `test_auc >= 0.778`，active runner / 默认实验口径是实验 110: `dual64x80 + branchBCE=0.18 + recompute_minibatch batch=65536 lr=3e-4`，四 seed mean `0.778321`、peak CUDA `6.19GB`。实验 104 是前一版 heavy default-training 对照，实验 106 是 heavy full-batch refinement 起点，实验 109 是较早的 full-batch 低成本双塔候选。后续默认以实验 110 为基准继续找 `~7GB` 显存水平下更稳定或更高峰值的 pure-CDM 信号；实验 99 的 hybrid stacker 三 seed mean `0.786910` 仍是最高绝对值但不是默认 CDM 路线；`0.780` 仍可作为 desirable headroom。
