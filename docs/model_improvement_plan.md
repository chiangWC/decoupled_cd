...
- 当前 low-memory pure-CDM training candidate: 实验 110，`dual64x80 + branchBCE=0.18` 改为 `recompute_minibatch`，`batch=65536`、`lr=3e-4`，四 seed mean AUC `0.778321`、peak CUDA `6.19GB`。实验 111 已按用户要求排除 checkpoint inference average，对 exp110 做完整 Track A-D 消融；最高 mean-AUC 变体是 `batch=131072, lr=3e-4`，mean AUC `0.778536`（相对 exp110 `+0.000215`）但 peak CUDA `8.76GB`，所以只作为更高显存候选。exp110 仍是最佳低显存口径；最高单 seed 是 `c_prior_start001 seed2027 = 0.779377`，但四 seed mean 基本中性。实验 111 同时确认 branch BCE、cognitive alignment、dual tower 都是核心组件，移除后三者分别掉 `0.001840/0.003924/0.003665` AUC。
