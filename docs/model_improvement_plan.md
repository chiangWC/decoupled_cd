...
- 当前已吸收的最新结构更新:
  ...
  - 实验 93/94: 将同一 train-history evidence 改为 `loss_only` 训练目标，约束 cognitive logits 与固定 evidence prior 的标准化排序对齐；hot config `target_concept=0.44, alignment=0.08810` 在 seed2027 达到 `test_auc 0.776868`，相对 high-water `+0.004187`，但 seed2026 会崩溃到 `AUC 0.504198`。降低到 `alignment=0.05` 后，seeds 2024/2025/2026/2027 全部 AUC 正向，matched mean `AUC +0.004828`，且 `ACC/RMSE/Brier/ECE` 均值同向改善。这是当前最干净的纯 CDM trial candidate；默认化前仍需把配置固化成 trial runner，并确认不改变默认 run script
