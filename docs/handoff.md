...
- 诊断 2 表明实验 51 full-trigger readout expert residual 可能会压制部分后续语义更干净结构的边际表现: `q-conditioned local mastery` 和 `difficulty-weighted propagation` 放回实验 49 底座后单 seed 至少不伤整体误差/校准，但在实验 51 底座上转成明显回撤。后续新结构若在最新主线上轻微负向但语义足够干净，可优先追加一次实验 49 底座交叉复验；详细指标见 [docs/model_improvement_plan.md](./model_improvement_plan.md) 的“诊断 2”。
